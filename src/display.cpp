...
inline uint32_t count_bits(uint32_t n)
{
...
#else	// HAS_TURBO
	char strTurbo[16];
	uint32_t uBitCount = count_bits((uint32_t)gamepad->turboState.buttons);
	sprintf(strTurbo, " T%02lu-%02lu", gamepad->uShotCount, uBitCount);
	statusBar += strTurbo;
